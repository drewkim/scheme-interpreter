...
def eval_all(expressions, env, tail=False):
    ...
    if expressions == nil:
        ...
        while expressions:
            if expressions.second is nil and tail:
...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        while formals:
            ...
            if (not formals and vals) or (formals and not vals):
                raise SchemeError("")
        return child
...
class PrimitiveProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            ...
            raise SchemeError('exception raised')
    ...
    while expressions is not nil:
        ...
        if scheme_truep(test):
            ...
            if len(clause.second) > 1:
                return scheme_eval(Pair('begin', clause.second), env, True)
...
def scheme_optimized_eval(expr, env, tail=False):
    ...
    while isinstance(result, Thunk):
        ...
        if (scheme_symbolp(first) and first in SPECIAL_FORMS):
            ...
            try:
                result = scheme_apply(scheme_optimized_eval(first, env), rest.map(lambda x: scheme_optimized_eval(x, env)), env)
            except:
                raise SchemeError(" ")
    return result
